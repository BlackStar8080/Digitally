...
@push('styles')
    <style>
        ...
        /* ===== MYTHICAL 5 IMPROVEMENTS (CALM VERSION) ===== */
...
.candidate-card.selected .candidate-checkbox i {
    opacity: 1;
    transform: scale(1);
}
...
.candidate-card.selected .stat-item {
    background: white;
    border-color: #d1fae5;
}
...
.candidate-card.selected .stat-value {
    color: #10b981;
}
...
.candidate-card.selected .mvp-score-badge {
    background: #d1fae5;
    color: #059669;
}
...
.btn-save-mythical:hover:not(:disabled) {
    background: #059669;
    transform: translateY(-1px);
    box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3);
}

.btn-save-mythical:disabled {
    background: #cbd5e1;
    cursor: not-allowed;
}
...
.btn-cancel-mythical:hover {
    border-color: #cbd5e1;
    color: #475569;
}

/* Display Mode Styles - Single Row */
.mythical-display {
    display: flex;
    gap: 1rem;
    overflow-x: auto;
...
.mythical-display::-webkit-scrollbar {
    height: 6px;
}

.mythical-display::-webkit-scrollbar-track {
    background: #f1f5f9;
    border-radius: 3px;
}

.mythical-display::-webkit-scrollbar-thumb {
    background: #cbd5e1;
    border-radius: 3px;
}

.mythical-player-card {
    background: white;
    border-radius: 12px;
    overflow: hidden;
...
<div class="info-card">
    ...
    <div class="mythical-section">
        <div class="mythical-header">
            <h5 class="mythical-title">
                <span class="mythical-title-icon">⭐</span>
                Mythical 5 
            </h5>
            <p class="mythical-subtitle">
        ...
        @if(empty($tournament->mythical_five) || (isset($editing) && $editing))
            ...
            <div class="selection-container">
                @if($mythicalCandidates->isEmpty())
                    <div class="mythical-empty">
                        <i class="bi bi-clipboard-data"></i>
                        <p>No player statistics available yet. Complete games to generate candidates.</p>
                    </div>
                @else
                    <form method="POST" action="{{ route('tournaments.mythical-five.save', $tournament->id) }}" id="mythicalForm">
                        @csrf
                        
                        <div class="selection-instructions">
                            <p>
                                <i class="bi bi-info-circle-fill"></i>
                                Click on player cards to select your Mythical 5. Top 7 performers shown. Scroll horizontally to see all.
                            </p>
                        </div>

                        <div class="selection-progress">
                            <div class="progress-indicator" id="selectionProgress">
                        ...
                        <div class="candidates-grid">
                            @foreach($mythicalCandidates as $index => $candidate)
                                @php
                                ...
                                <div class="candidate-card {{ $selected ? 'selected' : '' }}" 
                                    ...
                                    <input type="checkbox" 
                                           name="players[]" 
                                           value="{{ $player->id }}" 
                                           class="mythical-checkbox d-none" 
                                           {{ $selected ? 'checked' : '' }}
                                    ...
                                    <div class="candidate-header">
                                        <div class="candidate-top">
                                            ...
                                            <div class="candidate-rank">Top {{ $index + 1 }}</div>
                                        </div>
                                        <div class="candidate-info">
                                            <div class="candidate-name">{{ $player->name ?? 'Player' }}</div>
                                            <div class="candidate-team">{{ optional($player->team)->team_name ?? 'N/A' }}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="candidate-stats">
                                        <div class="stat-item">
                                            ...
                                            <div class="stat-value">{{ $candidate->total_points ?? 0 }}</div>
                                        </div>
                                        <div class="stat-item">
                                            <div class="stat-label">AST</div>
                                            <div class="stat-value">{{ $candidate->total_assists ?? 0 }}</div>
                                        </div>
                                        <div class="stat-item">
                                            <div class="stat-label">STL</div>
                                            <div class="stat-value">{{ $candidate->total_steals ?? 0 }}</div>
                                        </div>
                                        <div class="stat-item">
                                            <div class="stat-label">REB</div>
                                            <div class="stat-value">{{ $candidate->total_rebounds ?? 0 }}</div>
                                        </div>
                                    </div>

                                    <div class="mvp-score-badge">
                                        <div class="mvp-score-label">Score</div>
                                        <div class="mvp-score-value">{{ number_format($candidate->mvp_score ?? 0, 1) }}</div>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="selection-actions">
                            <button type="submit" class="btn btn-save-mythical" id="saveMythicalBtn" disabled>
                                <i class="bi bi-check-circle-fill"></i>
                                Save Mythical 5
                            </button>
                            @if(isset($editing) && $editing)
                                <a href="{{ route('tournaments.show', $tournament->id) }}" class="btn btn-cancel-mythical">
                                    <i class="bi bi-x-circle"></i>
                                    Cancel
                                </a>
                            @endif
        ...
        @else
            {{-- Display Mode --}}
            <div class="mythical-display">
                @php $stats = $selectedStats ?? collect(); @endphp
                @foreach($selectedPlayers as $player)
                    @php $s = $stats->get($player->id); @endphp
                    <div class="mythical-player-card">
                        <div class="player-card-header">
                            <div class="player-avatar">
                                {{ strtoupper(substr($player->name, 0, 1) ?? 'P') }}
                            </div>
                            <div class="player-card-name">{{ $player->name }}</div>
                            <div class="player-card-team">{{ optional($player->team)->team_name ?? 'N/A' }}</div>
                        </div>
                        <div class="player-card-body">
                            <div class="player-stats-grid">
                                <div class="player-stat-box">
                                    <div class="player-stat-label">PTS</div>
                                    <div class="player-stat-value">{{ $s->total_points ?? 0 }}</div>
                                </div>
                                <div class="player-stat-box">
                                    <div class="player-stat-label">AST</div>
                                    <div class="player-stat-value">{{ $s->total_assists ?? 0 }}</div>
                                </div>
                                <div class="player-stat-box">
                                    <div class="player-stat-label">STL</div>
                                    <div class="player-stat-value">{{ $s->total_steals ?? 0 }}</div>
                                </div>
                                <div class="player-stat-box">
                                    <div class="player-stat-label">REB</div>
                                    <div class="player-stat-value">{{ $s->total_rebounds ?? 0 }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
...
@endif
    ...
    @push('scripts')
        <script>
            (function() {
                const form = document.getElementById('mythicalForm');
                if (!form) return;

                const candidateCards = Array.from(document.querySelectorAll('.candidate-card'));
                const saveBtn = document.getElementById('saveMythicalBtn');
                const selectedCountEl = document.getElementById('selectedCount');
                const progressIndicator = document.getElementById('selectionProgress');

                // Use a Set to track selections reliably
                const selectedIds = new Set();

                // Create or remove hidden inputs to submit. Prefer updating original checkbox if present.
                function ensureHiddenInput(playerId) {
                    // If original checkbox exists (id=player_{id}), use it
                    const orig = document.getElementById('player_' + playerId);
                    if (orig) {
                        orig.checked = true;
                        return;
                    }

                    let input = form.querySelector(`input[type="hidden"][name="players[]"][value="${playerId}"]`);
                    if (!input) {
                        input = document.createElement('input');
                        input.type = 'hidden';
                        input.name = 'players[]';
                        input.value = playerId;
                        input.className = 'mythical-hidden';
                        form.appendChild(input);
                    }
                }

                function removeHiddenInput(playerId) {
                    const orig = document.getElementById('player_' + playerId);
                    if (orig) {
                        orig.checked = false;
                        return;
                    ...
                    const input = form.querySelector(`input[type="hidden"][name="players[]"][value="${playerId}"]`);
                ...
                function updateSelectionUI() {
                    ...
                    if (progressIndicator) {
                        if (count === 5) progressIndicator.classList.add('complete');
                        else progressIndicator.classList.remove('complete');
                    }

                    if (saveBtn) {
                        saveBtn.disabled = count !== 5;
                        if (count === 5) saveBtn.innerHTML = '<i class="bi bi-check-circle-fill"></i> Save Mythical 5';
                        else if (count < 5) saveBtn.innerHTML = `<i class="bi bi-star"></i> Select ${5 - count} More`;
                        else saveBtn.innerHTML = '<i class="bi bi-exclamation-circle"></i> Too Many Selected';
                    }

                    // Toggle visual state on candidate cards
                    candidateCards.forEach(card => {
                        const pid = String(card.getAttribute('data-player-id'));
                        if (selectedIds.has(pid)) card.classList.add('selected');
                        else card.classList.remove('selected');
                    });
                }

                // Initialize selectedIds from any pre-checked hidden inputs or pre-selected cards
                (function initSelected() {
                    // From existing original checkboxes (server-rendered) or hidden inputs
                    // Only initialize from inputs that are either checked (original checkboxes)
                    // or hidden inputs created previously. This avoids selecting all candidate inputs.
                    Array.from(form.querySelectorAll('input[name="players[]"]')).forEach(inp => {
                        if (!inp.value) return;
                        if (inp.type === 'hidden' || inp.checked) {
                            selectedIds.add(String(inp.value));
                        }
                    });

                    // Also consider cards marked selected in server-render (ensure inputs exist)
                    candidateCards.forEach(card => {
                        const pid = String(card.getAttribute('data-player-id'));
                        if (card.classList.contains('selected')) {
                            selectedIds.add(pid);
                            ensureHiddenInput(pid);
                        }
                    });
                ...
                // Toggle handler
                window.togglePlayerSelection = function(cardElement, playerId) {
                    const pid = String(playerId);
                    if (!selectedIds.has(pid) && selectedIds.size >= 5) {
                        // feedback
                        cardElement.style.animation = 'shake 0.45s';
                        setTimeout(() => cardElement.style.animation = '', 460);
                        showToast('Maximum 5 players can be selected', 'warning');
                        return;
                    }

                    if (selectedIds.has(pid)) {
                        selectedIds.delete(pid);
                        removeHiddenInput(pid);
                    } else {
                        selectedIds.add(pid);
                        ensureHiddenInput(pid);
                    }
                ...
                form.addEventListener('submit', function(e) {
                    if (selectedIds.size !== 5) {
                        e.preventDefault();
                        showToast('Please select exactly 5 players', 'error');
                        return false;
                    }
                ...
                function showToast(message, type = 'info') {
                    const toastClass = type === 'success' ? 'alert-success' : type === 'error' ? 'alert-danger' : type === 'warning' ? 'alert-warning' : 'alert-info';
