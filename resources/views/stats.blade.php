                                    ...
                                    <div class="performer-card {{ $cardClass }}">
                                        ...
                                        <div class="performer-info">
                                            ...
                                            <div class="performer-details">
                                                ...
                                                <small style="opacity: 0.7;">{{ $player->games_played ?? 0 }} Games
                                                    Played</small>
                                            </div>
                                        </div>
                                        <div class="performer-stats">
                                            @if ($isVolleyball)
                                                <div class="performer-stat">
                                                    <span
                                                        class="performer-stat-value">{{ $player->avg_kills ?? '0.0' }}</span>
                                                    <span class="performer-stat-label">KPG</span>
                                                </div>
                                                <div class="performer-stat">
                                                    <span
                                                        class="performer-stat-value">{{ $player->avg_aces ?? '0.0' }}</span>
                                                    <span class="performer-stat-label">APG</span>
                                                </div>
                                                <div class="performer-stat">
                                                    <span
                                                        class="performer-stat-value">{{ $player->avg_blocks ?? '0.0' }}</span>
                                                    <span class="performer-stat-label">BPG</span>
                                                </div>
                                            @else
                                                <div class="performer-stat">
                                                    <span
                                                        class="performer-stat-value">{{ $player->avg_points ?? '0.0' }}</span>
                                                    <span class="performer-stat-label">PPG</span>
                                                </div>
                                                <div class="performer-stat">
                                                    <span
                                                        class="performer-stat-value">{{ $player->avg_assists ?? '0.0' }}</span>
                                                    <span class="performer-stat-label">APG</span>
                                                </div>
                                                <div class="performer-stat">
                                                    <span
                                                        class="performer-stat-value">{{ $player->avg_rebounds ?? '0.0' }}</span>
                        ...
                        <table class="stats-table" id="statsTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>Player</th>
                                    @if ($isVolleyball)
                                        <th class="sortable" data-column="kills">
                                            <span class="stat-icon">🔥</span> KPG
                                        </th>
                                        <th class="sortable" data-column="aces">
                                            <span class="stat-icon">⚡</span> APG
                                        </th>
                                        <th class="sortable" data-column="blocks">
                                            <span class="stat-icon">🚫</span> BPG
                                        </th>
                                        <th class="sortable" data-column="digs">
                                            <span class="stat-icon">🏐</span> DPG
                                        </th>
                                        <th class="sortable" data-column="assists">
                                            <span class="stat-icon">🤝</span> SPG
                                        </th>
                                        <th class="sortable" data-column="errors">
                                            <span class="stat-icon">❌</span> EPG
                                        </th>
                                    @else
                                        ...
                                        <th class="sortable" data-column="steals">
                                            <span class="stat-icon">🚫</span> SPG
                                        </th>
                                    ...
                                    <tr>
                                        <td>
                                            @php
                                                $rankClass = 'default';
                                                $actualRank =
                                                    ($playerStats->currentPage() - 1) * $playerStats->perPage() +
                                                    $index +
                                                    1;
                                                if ($actualRank === 1) {
                                                    $rankClass = 'gold';
                                        ...
                                        <td>
                                            <div class="player-info">
                                                ...
                                                <div class="player-details">
                                                    ...
                                                    <span
                                                        class="player-team">{{ $player->team->team_name ?? 'No Team' }}</span>
                                                    <span class="games-played">{{ $player->games_played ?? 0 }}
                                                        games</span>
                                                </div>
                                            </div>
                                        </td>
                                        @if ($isVolleyball)
                                            <td>
                                                <span class="stat-value">
                                                    {{ number_format($player->avg_kills ?? 0, 1) }}
                                                </span>
                                            </td>
                                            <td>
                                                <span class="stat-value">
                                                    {{ number_format($player->avg_aces ?? 0, 1) }}
                                                </span>
                                            </td>
                                            <td>
                                                <span class="stat-value">
                                            ...
                                            <td>
                                                <span class="stat-value">
                                                    {{ number_format($player->avg_digs ?? 0, 1) }}
                                                </span>
                                            </td>
                                            <td>
                                                <span class="stat-value">
                                            ...
                                            <td>
                                                <span class="stat-value">
                                                    {{ number_format($player->avg_errors ?? 0, 1) }}
                                                </span>
                                            </td>
                                        @else
                                            <td>
                                            ...
                                            <td>
                                                <span class="stat-value">
                                                    {{ number_format($player->avg_steals ?? 0, 1) }}
                                                </span>
                                            </td>
                                            <td>
                                                <span class="stat-value">
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const isVolleyball = {{ $isVolleyball ? 'true' : 'false' }};

            headers.forEach(header => {
                header.addEventListener('click', function() {
                    const column = this.dataset.column;
                    const tbody = table.querySelector('tbody');
                    const rows = Array.from(tbody.querySelectorAll('tr')).filter(row => !row
                    ...
                    // Get column index based on the stat type and sport
                    let columnMap;
                    if (isVolleyball) {
                        columnMap = {
                            'kills': 2,
